...
describe('JobExecutor', () => {
  let repository: {
    ...
    markRunTimedOut: ReturnType<typeof vi.fn>;
  };
  ...
  it('executes a queued job successfully', async () => {
    ...
    expect(repository.markRunTimedOut).not.toHaveBeenCalled();
  });
  ...
  it('marks a run as failed when the handler throws', async () => {
    repository.findRunById.mockResolvedValue(queuedRun);
    ...
    repository.markRunFailed.mockResolvedValue(failedRun);
  ...
  it('rejects invalid handler output', async () => {
    ...
    repository.markRunFailed.mockResolvedValue({
      ...claimedRun,
      status: 'failed',
      finishedAt: new Date('2026-09-24T12:01:00.000Z'),
